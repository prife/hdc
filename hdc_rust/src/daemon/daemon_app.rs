...
impl AppTaskMap {
    ...
    pub async fn get(session_id: u32, channel_id: u32) -> Option<DaemonAppTask_> {
        let arc = Self::get_instance();
        let map = arc.lock().await;
        let task = match map.get(&(session_id, channel_id)) {
            Some(task) => task,
            None => {
                hdc::error!("session_id:{} channel_id:{} not found", session_id, channel_id);
                return None
            }
        };
        Some(task.clone())
...
async fn do_app_check(session_id: u32, channel_id: u32, _payload: &[u8]) -> bool {
    let arc = match AppTaskMap::get(session_id, channel_id).await {
        Some(task) => task,
        None => {
            hdc::error!("session_id:{} channel_id:{} not found", session_id, channel_id);
            return false
...
async fn app_uninstall(session_id: u32, channel_id: u32, _payload: &[u8]) {
    let mut str = match String::from_utf8(_payload.to_vec()) {
        Ok(v) => v,
        Err(e) => {
            hdc::error!("session_id:{} channel_id:{} uninstall error:{}", session_id, channel_id, e);
            put_app_finish(session_id, channel_id, 0, 0, &mut e.to_string().into_bytes()[..]).await;
            return;
        }
    };
...
async fn handle_execute_result(
    ...
    match &result {
        ...
        Err(err) => {
            hdc::error!("session_id:{} channel_id:{} error:{}", session_id, channel_id, err);
            put_app_finish(
                session_id,
                channel_id,
                mode,
    ...
    match result {
        Ok(output) => {
            ...
            let mut str = match String::from_utf8(msg) {
                Ok(s) => s,
                Err(e) => {
                    hdc::error!("cmd execute error: {}", e);
                    return Err(Error::new(
                        ErrorKind::Other,
                        format!("cmd execute error: {}", e),
                    ))
                }
            };
            str = str.replace('\n', " ");
